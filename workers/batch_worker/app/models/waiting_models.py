...
class WaitingTimeWeatherSplitter(WaitingTimeSplitter):
    def __init__(self, dataset: pd.DataFrame, direction):
        super().__init__(dataset, direction)

    def __split_for_cars(self, prepared_train_df):
        prepared_train_df_nb_cars = prepared_train_df[["ds", f"nb_cars_{self.direction}"], "avg_temperature", "pressure", "precipitation_MM", "windspeed_Kmph"].copy()
        prepared_train_df_nb_cars.rename(columns={f'nb_cars_{self.direction}':'y'}, inplace=True)
        return prepared_train_df_nb_cars

    def __split_for_trucks(self, prepared_train_df):
        prepared_train_df_nb_trucks = prepared_train_df[["ds", f"nb_trucks_{self.direction}"], "avg_temperature", "pressure", "precipitation_MM", "windspeed_Kmph"].copy()
        prepared_train_df_nb_trucks.rename(columns={f'nb_trucks_{self.direction}':'y'}, inplace=True)
        return prepared_train_df_nb_trucks



class WaitingTimeWeatherPredictor(WaitingTimePredictor):
    def __init__(self, direction, is_fitted=False, prophet_waiting_time=None, prophet_nb_cars=None,
                 prophet_nb_trucks=None):
        super().__init__(direction, is_fitted=False, prophet_waiting_time=None, prophet_nb_cars=None,
                 prophet_nb_trucks=None)
        self.prophet_waiting_time = prophet_waiting_time or Prophet()
        self.prophet_nb_cars = prophet_nb_cars or Prophet()
        self.prophet_nb_trucks = prophet_nb_trucks or Prophet()
        ...
        self.__is_fitted = is_fitted
        if not self.__is_fitted:
            self.add_regressor("avg_temperature")
            self.add_regressor("pressure")
            self.add_regressor("precipitation_MM")
            self.add_regressor("windspeed_Kmph")

    def fit(self, dataset, train_ratio=0.8):
        if self.__is_fitted:
            raise ValueError("Model already fitted.")
        dataset_splitter = WaitingTimeWeatherSplitter(dataset, self.direction)
        waiting_train_df, cars_train_df, trucks_train_df = dataset_splitter.split(train_percentage=train_ratio,
                                                                                  include_test=False)
        self.prophet_waiting_time.fit(waiting_train_df)
        self.prophet_nb_cars.fit(cars_train_df)
